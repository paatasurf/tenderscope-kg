...
assert sum([
    _W_CAPABILITY, _W_BUYER_HISTORY, _W_INDUSTRY_HISTORY, _W_VALUE_FIT,
    _W_GEO_FIT, _W_COMPETITION, _W_BUYER_ATTRACT, _W_STRATEGIC,
    _W_WORKLOAD, _W_WIN_PROBABILITY,
]) == 100
...
def _company_tenders(repo: BizRepository, company_uid: str) -> list[dict]:
    ...
    for rel, ent in repo.get_neighbors(
        company_uid, direction="out", kinds=_BID_KINDS, limit=5000
    ):
...
def _tender_buyer(repo: BizRepository, tender_uid: str) -> Optional[dict]:
    ...
    for rel, ent in repo.get_neighbors(
        ...
        kinds=[BizRelationKind.ISSUED_BY], limit=5
    ):
        if ent.kind in _BUYER_KINDS:
            return {"uid": ent.uid, "name": ent.name,
                    "kind": ent.kind.value, "attributes": ent.attributes}
    return None


def _tender_competitors(repo: BizRepository, tender_uid: str,
                        exclude_uid: str) -> list[dict]:
...
def _company_industries(repo: BizRepository, company_uid: str) -> list[dict]:
    ...
    results = []
    for rel, ent in repo.get_neighbors(
        company_uid, direction="out",
        kinds=[BizRelationKind.IN_INDUSTRY], limit=100
    ):
...
def _company_locations(repo: BizRepository, company_uid: str) -> set[str]:
    ...
    for rel, ent in repo.get_neighbors(
        ...
        kinds=[BizRelationKind.IN_CITY, BizRelationKind.IN_PROVINCE,
               BizRelationKind.LOCATED_AT, BizRelationKind.HAS_ADDRESS],
        limit=100
    ):
        locs.add(ent.uid)
...
def _tender_value(attrs: dict) -> Optional[float]:
    for k in ("value", "contract_value", "estimated_value",
              "budget", "award_value", "amount"):
...
def _tender_date(attrs: dict) -> Optional[str]:
    for k in ("valid_from", "date", "issue_date", "posted_date",
...
def _deadline_date(attrs: dict) -> Optional[str]:
    for k in ("closing_date", "deadline", "due_date",
              "submission_deadline", "valid_to"):
        v = attrs.get(k)
        if v:
            return str(v)
    return None
...
class OpportunityIntelligenceEngine:
    ...
    def _score_capability(self, company_uid: str, tender: dict) -> tuple[float, list]:
        ...
        for ind in tender_inds:
            if ind["uid"] in company_inds:
                overlap += 1
                evidence.append(_ev(
                    company_uid, "in_industry", ind["uid"],
                    "", ind["name"]
                ))
        ...
        for ct in company_tenders:
            ...
            if ct_inds & tender_inds:
                ...
                if ct["is_win"]:
                    relevant_wins += 1
                    evidence.append(_ev(
                        company_uid, "won_similar_industry_tender",
                        ct["uid"], "", ct["name"]
                    ))
    ...
    def _score_value_fit(self, company_uid: str, tender_value: Optional[float]) -> tuple[float, list]:
        ...
        if match_count > 0:
            evidence.append(_ev(
                company_uid, "value_fit", tender_bucket or "unknown",
    ...
    def _score_geo_fit(self, company_uid: str, tender: dict) -> tuple[float, list]:
        ...
        if tender_city and tender_city in company_locs:
            score = 1.0
            evidence.append(_ev(company_uid, "in_city", tender_city,
                                "", f"Company operates in {tender_city}"))
        elif tender_prov and tender_prov in company_locs:
            score = 0.7
            evidence.append(_ev(company_uid, "in_province", tender_prov,
    ...
    def _score_competition(self, tender_uid: str) -> tuple[float, list]:
        ...
        if n > 0:
            evidence.append(_ev(
                tender_uid, "competitors", str(n),
                "", f"{n} competitors on this tender"
            ))
        return round(score, 4), evidence
    ...
    def _score_buyer_attractiveness(self, buyer: Optional[dict]) -> tuple[float, list]:
        ...
        evidence.append(_ev(
            buyer_uid, "issues_tenders", str(n_tenders),
            buyer["name"], f"{n_tenders} historical tenders"
        ))
        return round(score, 4), evidence

    def _score_strategic(self, company_uid: str, buyer: Optional[dict],
                         tender_inds: list[dict]) -> tuple[float, list]:
        """
        ...
        if buyer:
            ...
            if not overlap:
                ...
                evidence.append(_ev(
                    company_uid, "new_buyer_opportunity",
                    buyer["uid"], "", buyer["name"]
                ))
            else:
        ...
        for ind in tender_inds:
            if ind["uid"] not in company_inds:
                ...
                evidence.append(_ev(
                    company_uid, "new_industry_expansion",
                    ind["uid"], "", ind["name"]
                ))
    ...
    def _score_workload(self, company_uid: str) -> tuple[float, list]:
        ...
        recent = [
            t for t in company_tenders
            if _parse_year(_tender_date(t["attributes"])) in (current_year, current_year - 1)
        ]
        ...
        evidence.append(_ev(
            company_uid, "recent_tender_count", str(n),
            "", f"{n} tenders in last 24 months"
        ))
        return round(score, 4), evidence

    def _score_win_probability(self, company_uid: str, buyer: Optional[dict],
                               tender_uid: str) -> tuple[float, list]:
        """
        ...
        if buyer:
            ...
            if buyer_bids:
                ...
                evidence.append(_ev(
                    company_uid, "buyer_specific_win_rate",
                    buyer["uid"], "",
                    f"{len(buyer_wins)}/{len(buyer_bids)} wins with this buyer"
                ))

        evidence.append(_ev(
            company_uid, "overall_win_rate", str(total),
            "", f"{len(wins)}/{total} overall wins"
        ))
    ...
    def opportunity_score(self, company_uid: str, tender_uid: str) -> dict:
        ...
        for ev_list in [ev_cap, ev_bh, ev_ih, ev_val, ev_geo,
                        ev_comp, ev_ba, ev_str, ev_wl, ev_wp]:
        ...
        return {
            ...
            "max_score":      100,
            "dimensions": {
                "capability_fit":      {"score": round(s_cap * _W_CAPABILITY, 1),  "raw": s_cap,  "weight": _W_CAPABILITY},
                "buyer_history":       {"score": round(s_bh  * _W_BUYER_HISTORY, 1), "raw": s_bh, "weight": _W_BUYER_HISTORY},
                "industry_history":    {"score": round(s_ih  * _W_INDUSTRY_HISTORY, 1), "raw": s_ih, "weight": _W_INDUSTRY_HISTORY},
                "value_fit":           {"score": round(s_val * _W_VALUE_FIT, 1),   "raw": s_val,  "weight": _W_VALUE_FIT},
                "geographic_fit":      {"score": round(s_geo * _W_GEO_FIT, 1),     "raw": s_geo,  "weight": _W_GEO_FIT},
                "competition_level":   {"score": round(s_comp * _W_COMPETITION, 1),"raw": s_comp, "weight": _W_COMPETITION},
                "buyer_attractiveness":{"score": round(s_ba  * _W_BUYER_ATTRACT, 1),"raw": s_ba,  "weight": _W_BUYER_ATTRACT},
                "strategic_importance":{"score": round(s_str * _W_STRATEGIC, 1),   "raw": s_str,  "weight": _W_STRATEGIC},
                "workload_impact":     {"score": round(s_wl  * _W_WORKLOAD, 1),    "raw": s_wl,   "weight": _W_WORKLOAD},
                "win_probability":     {"score": round(s_wp  * _W_WIN_PROBABILITY, 1),"raw": s_wp,"weight": _W_WIN_PROBABILITY},
            },
    ...
    def opportunity_risk(self, company_uid: str, tender_uid: str) -> dict:
        ...
        if comp_raw < 0.4:
            risks.append({"factor": "high_competition", "severity": "high",
        ...
        if cap_raw < 0.3:
            risks.append({"factor": "capability_gap", "severity": "high",
            ...
            risks.append({"factor": "partial_capability", "severity": "medium",
                          "detail": "Partial capability overlap — some gaps present"})

        # Workload risk
        wl_raw = dims["workload_impact"]["raw"]
        if wl_raw < 0.4:
            risks.append({"factor": "high_workload", "severity": "medium",
        ...
        if bh_raw < 0.25:
            risks.append({"factor": "no_buyer_history", "severity": "medium",
        ...
        if val_raw < 0.3:
            risks.append({"factor": "value_mismatch", "severity": "medium",
        ...
        if geo_raw < 0.4:
            risks.append({"factor": "geographic_stretch", "severity": "low",
        ...
        if tl.get("deadline_risk") == "high":
            risks.append({"factor": "tight_deadline", "severity": "high",
    ...
    def portfolio_impact(self, company_uid: str, tender_uid: str) -> dict:
        ...
        future_potential = (
            "high"   if buyer_volume >= 10 else
            "medium" if buyer_volume >= 3 else
            "low"
        )

        evidence: list[dict] = []
        if buyer:
            evidence.append(_ev(company_uid, "buyer_volume", buyer["uid"],
    ...
    def similar_opportunities(self, company_uid: str, tender_uid: str,
                              limit: int = 10) -> dict:
        """
        ...
        evidence = [
            _ev(company_uid, "similar_tender", r["uid"], company.name, r["name"])
    ...
    def best_opportunities(self, company_uid: str, limit: int = 10) -> dict:
        ...
        for t in all_tenders:
            ...
            if "error" in s:
                continue
            rec_label = self.opportunity_recommendation(company_uid, t.uid).get(
                "recommendation", "")
            scored_list.append({
                "tender_uid":     t.uid,
                "tender_name":    t.name,
    ...
    def executive_summary(self, company_uid: str, limit: int = 5) -> dict:
        ...
        ignore_top = sorted(
            [t for t in top if t["recommendation"] == "Ignore"],
        ...
        for opp in pursue_top[:3]:
            ...
            if risk.get("overall_risk") == "high":
                biggest_risks.append(f"{opp['tender_name']}: {', '.join(r['factor'] for r in risk.get('risk_factors', []) if r['severity'] == 'high')}")
            tl = self.opportunity_timeline(company_uid, opp["tender_uid"])
            if tl.get("submission_urgency") in ("critical", "high"):
                next_actions.append(f"URGENT: Start bid on '{opp['tender_name']}' ({tl.get('months_until_deadline')} months)")
            else:
